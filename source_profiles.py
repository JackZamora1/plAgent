...
class SourceProfileRegistry:
    ...
    def _register_builtin_profiles(self):
        ...
        # ============================================================
        # Note: The universal profile should handle all cases.
        # These are kept in case users explicitly request them.

        # OBITUARY PROFILE
        # ============================================================
        obituary_profile = SourceProfile(
            source_type="obituary",
            display_name="Chinese Military Obituary",
            source_description=(
                "formal obituary announcement from Chinese state media (Xinhua, CCTV, etc.). "
                "These are comprehensive biographical summaries published after an officer's death, "
                "typically including full career progression, family information, and political affiliations"
            ),
            extraction_context=(
                "Obituaries are the MOST COMPLETE source for biographical data. They typically include:\n"
                "- Full name, pinyin, hometown, birth/death dates\n"
                "- Complete career progression from enlistment through retirement\n"
                "- All major promotions with dates\n"
                "- Political participation (Party membership, Congress, CPPCC)\n"
                "- Awards and honors\n"
                "- Sometimes family information (wife name)\n\n"
                "Death date is EXPECTED to be present. If you don't see it, verify thoroughly."
            ),
            required_fields=["name", "death_date"],
            common_fields=[
                "pinyin_name", "hometown", "birth_date", "enlistment_date",
                "party_membership_date", "promotions", "notable_positions", "awards"
            ],
            rare_fields=[
                "wife_name", "retirement_date",
                "congress_participation", "cppcc_participation"
            ],
            field_expectations={
                "death_date": "ALWAYS present in obituaries - this is a memorial announcement",
                "enlistment_date": "Usually stated, sometimes only implicitly (e.g., '1961年入伍')",
                "promotions": "Explicit promotions with dates typically included",
                "notable_positions": "Full career arc usually detailed with 历任 (successively held)",
                "wife_name": "Rarely mentioned, but check carefully",
                "retirement_date": "Sometimes included, especially for recent retirees"
            },
            min_confidence_threshold=0.75,
            confidence_weights={
                "death_date": 2.0,      # Critical field for obituaries
                "enlistment_date": 1.5,
                "promotions": 1.5,
                "notable_positions": 1.5,
                "party_membership_date": 1.2,
                "hometown": 1.0,
                "birth_date": 1.0,
            },
            field_search_terms={
                "wife_name": ["妻子", "夫人", "配偶", "爱人", "伴侣"],
                "retirement_date": ["退休", "离休", "退役"],
                "congress_participation": ["全国代表大会", "党代会", "代表"],
                "cppcc_participation": ["政协", "全国委员会", "委员"]
            }
        )
        self.register(obituary_profile)

        # ============================================================
        # NEWS ARTICLE PROFILE
        # ============================================================
        news_profile = SourceProfile(
            source_type="news_article",
            display_name="News Article",
            source_description=(
                "news article or report from Chinese media covering a current event, "
                "appointment, promotion, or activity involving a PLA officer. "
                "These focus on recent events rather than comprehensive biographies"
            ),
            extraction_context=(
                "News articles are PARTIAL sources - they focus on current events, not full biographies.\n"
                "Typical content:\n"
                "- Officer name and current position (ALWAYS present)\n"
                "- Current rank and recent promotions (OFTEN present)\n"
                "- Hometown and age/birth year (SOMETIMES present)\n"
                "- Brief career highlights (SOMETIMES present)\n\n"
                "RARE in news articles:\n"
                "- Death date (officer is usually ALIVE)\n"
                "- Full career progression\n"
                "- Enlistment dates\n"
                "- Wife name or family information\n\n"
                "Set expectations accordingly - lower completeness is NORMAL for news sources."
            ),
            required_fields=["name"],
            common_fields=[
                "notable_positions",  # Current position almost always mentioned
                "hometown",
                "pinyin_name"
            ],
            rare_fields=[
                "death_date",        # Officer usually alive in news
                "enlistment_date",   # Historical dates rarely included
                "party_membership_date",
                "wife_name",
                "retirement_date",
                "full_career_arc"    # News focuses on current role, not history
            ],
            field_expectations={
                "notable_positions": "Current position almost always stated",
                "death_date": "Usually ABSENT - news covers living officers",
                "promotions": "Recent promotions may be mentioned, but historical ones unlikely",
                "hometown": "Sometimes included for context",
                "awards": "Major recent awards may be mentioned",
                "enlistment_date": "Rarely included in news articles"
            },
            min_confidence_threshold=0.65,  # Lower threshold for news - less complete is OK
            confidence_weights={
                "notable_positions": 2.0,  # Current position is critical for news
                "promotions": 1.5,
                "hometown": 1.0,
                "birth_date": 0.8,
                "enlistment_date": 0.5,    # Less important for news context
                "death_date": 0.3,         # Usually absent, not a problem
            },
            field_search_terms={
                "wife_name": ["妻子", "夫人", "配偶", "爱人"],
                "retirement_date": ["退休", "离休"],
                "death_date": ["逝世", "去世", "病逝", "辞世"],
                "congress_participation": ["全国代表大会", "党代会"],
                "cppcc_participation": ["政协", "委员"]
            }
        )
        self.register(news_profile)

        # ============================================================
        # WIKIPEDIA PROFILE
        # ============================================================
        wiki_profile = SourceProfile(
            source_type="wiki",
            display_name="Wikipedia Article",
            source_description=(
                "Wikipedia article (Chinese or English) about a PLA officer. "
                "These are encyclopedia entries with structured biographical information, "
                "often including detailed career timelines, awards, and political roles"
            ),
            extraction_context=(
                "Wikipedia articles vary in completeness but tend to be WELL-STRUCTURED:\n"
                "Typical content:\n"
                "- Name, pinyin, hometown (USUALLY in infobox)\n"
                "- Career highlights and notable positions (USUALLY detailed)\n"
                "- Promotions and awards (OFTEN present)\n"
                "- Political participation (OFTEN present)\n"
                "- Birth date (USUALLY present)\n\n"
                "Less common:\n"
                "- Enlistment dates (varies by article quality)\n"
                "- Party membership date (varies)\n"
                "- Death date (only if deceased)\n"
                "- Wife name (rarely included)\n\n"
                "Wikipedia articles may have INFOBOXES with structured data - pay special attention to these."
            ),
            required_fields=["name"],
            common_fields=[
                "pinyin_name", "hometown", "notable_positions",
                "promotions", "awards", "birth_date"
            ],
            rare_fields=[
                "enlistment_date",
                "party_membership_date",
            ...
            field_expectations={
                "pinyin_name": "Usually in infobox or first paragraph",
                "notable_positions": "Detailed career section typical",
                "promotions": "Often listed with dates",
                "awards": "Frequently documented",
                "enlistment_date": "Less common, varies by article quality",
                "death_date": "Present if officer is deceased"
            },
            min_confidence_threshold=0.70,
            confidence_weights={
                "notable_positions": 1.8,
                "promotions": 1.5,
                "awards": 1.3,
                "hometown": 1.2,
                "pinyin_name": 1.2,
                "birth_date": 1.0,
                "enlistment_date": 0.8,
                "party_membership_date": 0.8,
            },
            field_search_terms={
                "wife_name": ["妻子", "夫人", "配偶", "spouse", "wife"],
                "retirement_date": ["退休", "离休", "retired", "retirement"],
                "death_date": ["逝世", "去世", "died", "death"],
                "congress_participation": ["全国代表大会", "党代会", "Congress"],
                "cppcc_participation": ["政协", "委员", "CPPCC"]
            }
        )
        self.register(wiki_profile)
...
_registry = SourceProfileRegistry()


def get_profile(source_type: str) -> SourceProfile:
    """
    Get a source profile (convenience function).
...
def list_source_types() -> List[str]:
    """
    List all available source types (convenience function).
